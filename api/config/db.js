...
if (!connectionString) {
  console.warn('DATABASE_URL env var not set, database features will be disabled')
}
...
if (!isLibsql) {
  ...
  try {
    ...
    if (typeof db.pragma === 'function') {
      try {
        // 🛡️ CRITICAL OPTIMIZATIONS (from actly.md)
        db.pragma('journal_mode = WAL');
        db.pragma('synchronous = NORMAL');
        db.pragma('cache_size = -64000'); // 64MB cache
        db.pragma('foreign_keys = ON');
        console.log('⚡ [SQLITE] WAL mode and optimizations enabled');
      } catch (e) {
        console.warn('⚠️ [SQLITE] Optimization pragmas failed:', e.message);
      }
    }
  } catch (e) {
...
// 🛡️ Transaction Queue (from actly.md)
class TransactionQueue {
  constructor(pool) {
    this.pool = pool;
    this.buffer = [];
    this.flushing = false;
    this.flushInterval = 5000; // 5s batch flush
  ...
  push(sql, params = []) {
    ...
    if (this.buffer.length >= this.maxBatchSize) {
      this.flush().catch(err => console.error('[DB Queue] Immediate flush failed:', err));
    }
  }
  ...
  async flush() {
    ...
    try {
      await this.pool.batch(batch);
      console.log(`[DB Queue] Flushed ${batch.length} operations`);
    } catch (err) {
      console.error('[DB Queue] Batch insert failed:', err);
      // Put back in buffer (optional, depending on retry logic)
...
export const transactionQueue = new TransactionQueue(pool);
export { db };
