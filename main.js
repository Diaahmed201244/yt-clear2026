...
(function() {
    ...
    console.error = function(...args) {
        ...
        if (msg.includes('postMessage') && msg.includes('youtube.com')) return;
        if (msg.includes('Failed to execute') && msg.includes('DOMWindow')) return;
...
AppLifecycleManager.register("acc", {
  async init() {
    if (!window.FEATURES?.USE_ACC) {
        console.log('[ACC] Initializing...');
        const userId = localStorage.getItem('user_id');
        const token = localStorage.getItem('auth_token');
        
        if (userId && token) {
            console.log('[ACC] Found auth data, initializing with user:', userId);
            if (window.ACC && window.ACC.init) {
                return window.ACC.init(userId, token).catch(err => {
                    console.error('[ACC] Init failed:', err);
                    return Promise.resolve();
                });
            } else {
                console.warn('[ACC] ACC object not available');
                return Promise.resolve();
            }
        } else {
            console.log('[ACC] No auth data in localStorage, waiting for login...');
            // Listen for auth event
            window.addEventListener('auth:authenticated', (e) => {
                console.log('[ACC] Auth event received, late init...');
                if (window.ACC && window.ACC.init && e.detail?.userId) {
                    window.ACC.init(e.detail.userId, e.detail.token);
                }
            }, { once: true });
            return Promise.resolve();
        }
  ...
  async start() {
    if (!window.FEATURES?.USE_ACC) return;
...
AppLifecycleManager.register("bankode", {
  async start() {
    ...
    }, 30000); // 🛡️ 30 seconds polling for /api/sqlite/codes
  }
}, ["auth"]);
...
if (!window.__APP_STARTED__) {
  ...
  // 🛡️ DISABLED: Automatic page reloads to prevent infinite reload loops
  window.safeReload = function () {
    console.warn("🔁 Safe reload DISABLED - preventing reload loop");
  ...
  async function boot() {
    try { 
        ...
        window.addEventListener('auth:authenticated', async (e) => {
            ...
            console.log("🔐 [Auth] User authenticated, initializing ACC...");
            if (window.ACC) {
                await window.ACC.init(userId, token);
            }
        });
