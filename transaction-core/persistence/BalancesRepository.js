...
export class BalancesRepository extends BaseRepository {
  ...
  async applyTransfer(actor, target, amount, client) {
    ...
    await c.query(`UPDATE balances SET balance = balance - $2, updated_at = NOW() WHERE user_id = $1`, [actor, amount])
    await c.query(`UPDATE balances SET balance = balance + $2, updated_at = NOW() WHERE user_id = $1`, [target, amount])
  }

  async applyDebit(userId, amount, client) {
    const c = client || (this.client && this.client.pool)
    await this.upsertUser(userId, c)
    await c.query(`UPDATE balances SET balance = balance - $2, updated_at = NOW() WHERE user_id = $1`, [userId, amount])
  }
