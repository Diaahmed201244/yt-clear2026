...
function ConversationSkeleton() {
    return (<div className="flex-1 p-4 space-y-4">
      {[1, 2, 3, 4, 5].map((i) => (<div key={i} className={cn("flex gap-2", i % 2 === 0 ? "justify-end" : "justify-start")}>
...
<<<<<<< HEAD
    ...
    if (!activeChat) {
        return (<div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-muted/30">
        <div className="rounded-full bg-muted p-8 mb-6">
          <lucide_react_1.MessageCircle className="h-12 w-12 text-muted-foreground"/>
        </div>
    ...
    return (<div className="flex-1 flex flex-col h-full">
      <header className="flex items-center gap-3 px-4 h-16 border-b bg-background shrink-0">
        {isMobile && (<Button size="icon" variant="ghost" onClick={onBack} data-testid="button-back">
            <ArrowLeft className="h-5 w-5"/>
          </Button>)}

        <div className="relative">
          <Avatar className="h-10 w-10">
            <AvatarImage src={activeChat.avatar || (otherParticipant === null || otherParticipant === void 0 ? void 0 : otherParticipant.avatar)}/>
            <AvatarFallback className="bg-primary text-primary-foreground text-sm font-medium">
              {getInitials(activeChat.name)}
            </AvatarFallback>
          </Avatar>
          {(otherParticipant === null || otherParticipant === void 0 ? void 0 : otherParticipant.isOnline) && (<span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full bg-status-online border-2 border-background"/>)}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            ? "typing..."
            : (otherParticipant === null || otherParticipant === void 0 ? void 0 : otherParticipant.isOnline)
                ? "Online"
                : (otherParticipant === null || otherParticipant === void 0 ? void 0 : otherParticipant.lastSeen)
                    ? `Last seen ${format(otherParticipant.lastSeen, "HH:mm")}`
                    : "Offline"}
          </p>
        </div>
      ...
      {isLoading ? (<ConversationSkeleton />) : messages.length === 0 ? (<EmptyConversation />) : (<ScrollArea className="flex-1">
          <div ref={parentRef} className="h-full overflow-auto">
            <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: "relative" }}>
              {rowVirtualizer.getVirtualItems().map((vRow) => {
                const item = messagesWithDateSeparators[vRow.index];
                const index = vRow.index;
                if (item.type === "date" && item.date) {
                    return (<div key={`date-${item.date.getTime()}`} style={{ position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${vRow.start}px)` }} className="flex justify-center py-4">
                ...
                if (item.type === "message" && item.message) {
                    ...
                    const prevItem = messagesWithDateSeparators[index - 1];
                    const prevMessage = (prevItem === null || prevItem === void 0 ? void 0 : prevItem.type) === "message" ? prevItem.message : null;
                    ...
                    const replyToMessage = message.replyTo
                        ...
                        : undefined;
                    return (<div key={message.id} style={{ position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${vRow.start}px)` }}>
                      <MessageBubble message={message} isGrouped={isGrouped} showAvatar={showAvatar} replyToMessage={replyToMessage}/>
                    </div>);
                }
                return null;
              })}
            ...
            <div className="p-2">
              <TypingIndicator typingUsers={typingUsersInActiveChat}/>
            </div>
          </div>
        </ScrollArea>)}
