...
<<<<<<< HEAD
    ...
    if (!res.ok || !json.ok) throw new Error('FEED_FAILED')
    return json.videos || json.items || []
  },
  async trending() {
    const res = await fetch('/api/farragna/trending', { credentials: 'include' })
    const json = await res.json().catch(() => ({ ok: false }))
    if (!res.ok || !json.ok) throw new Error('TRENDING_FAILED')
    return json.videos || []
=======
    try {
      const res = await fetch('/api/farragna/feed', { credentials: 'include' })
...
<<<<<<< HEAD
  views.innerHTML = `👁️ ${v.views_count||0}`
  const rewards = document.createElement('div')
  rewards.className = 'pill'
  rewards.innerHTML = `🪙 ${v.rewards_earned||0}`
=======
...
<<<<<<< HEAD
  const ready = (list||[]).filter(v => v.status === 'ready' && v.playback_url)
=======
...
<<<<<<< HEAD
    ...
    try {
      const { upload_url } = await api.requestUpload()
      showStatus('Uploading to Cloudflare...')
      ...
      showStatus('Processing... The video will appear when ready')
=======
