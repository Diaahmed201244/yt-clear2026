...
import fs from "fs";
<<<<<<< HEAD
import { CLOUDFLARE_ACCOUNT_ID, MAX_VIDEO_SIZE, VIDEO_CATEGORIES, ENGAGEMENT_TYPES, MODERATION_STATUSES as MOD_STATUSES, VIDEO_STATUSES } from "./config";
=======
...
export async function registerRoutes(httpServer, app) {
    ...
    app.post("/api/videos", async (req, res) => {
        try {
<<<<<<< HEAD
            console.log("🔥 FARRAGNA URL UPLOAD HIT - GUEST MODE");
            ...
            const videoData = sharedSchema.insertVideoSchema.parse({
                ...req.body,
                userId: fakeUserId, // Fake user ID assigned
                // uploaderType: "guest", // Temporarily commented out until DB migration
            ...
            await storage.createConsentRecord({
                videoId: video.id,
                userId: fakeUserId,
                copyrightAgreed: req.body.copyrightConsent,
                responsibilityAgreed: req.body.responsibilityConsent,
                youtubeRightsAgreed: !!req.body.youtubeUrl,
                ipAddress: req.ip,
                userAgent: req.get("user-agent"),
            });
            console.log(`✅ Guest video created as ${fakeUserId}: ${video.id}`);
