...
class EnemyManager {
    ...
    createSniper(x, y) {
        return {
            ...
            health: 80,
            maxHealth: 80,
            type: enemies_1.EnemyType.SNIPER,
            lastAttack: 0,
            attackCooldown: 3000,
            isActive: true,
            damage: 50
    ...
    createPatrolEnemy(x, y, patrolPath) {
        return {
            ...
            health: 120,
            maxHealth: 120,
            type: enemies_1.EnemyType.PATROL,
            lastAttack: 0,
            attackCooldown: 1500,
            patrolPath,
            currentPathIndex: 0,
    ...
    createHunter(x, y) {
        return {
            ...
            health: 150,
            maxHealth: 150,
            type: enemies_1.EnemyType.HUNTER,
            lastAttack: 0,
            attackCooldown: 1000,
            isActive: true,
            damage: 40
    ...
    createWall(x, y, width, height) {
        return {
            ...
            width,
            height,
            type: enemies_1.ObstacleType.WALL,
    ...
    createSpikes(x, y, width, height) {
        return {
            ...
            width,
            height,
            type: enemies_1.ObstacleType.SPIKES,
            isDestructible: false,
            damage: 35,
            effect: 'slow',
            effectDuration: 2000
    ...
    createLaserBeam(x, y, width, height) {
        return {
            ...
            width,
            height,
            type: enemies_1.ObstacleType.LASER_BEAM,
            isDestructible: false,
            damage: 60,
            effect: 'stun',
            effectDuration: 1500
    ...
    createElectricFence(x, y, width, height) {
        return {
            ...
            width,
            height,
            type: enemies_1.ObstacleType.ELECTRIC_FENCE,
            isDestructible: false,
            damage: 40,
            effect: 'stun',
            effectDuration: 2000
    ...
    // Power-up Creation Methods
    createSpeedBoost(x, y) {
        return {
            id: `speed_${Date.now()}_${Math.random()}`,
            x,
            y,
            type: enemies_1.PowerUpType.SPEED_BOOST,
    ...
    createShield(x, y) {
        return {
            ...
            x,
            y,
            type: enemies_1.PowerUpType.SHIELD,
    ...
    updateEnemies(playerX, playerY, deltaTime) {
        this.enemies.forEach(enemy => {
            if (!enemy.isActive)
                return;
            switch (enemy.type) {
                case enemies_1.EnemyType.GUARD:
                    this.updateGuard(enemy, playerX, playerY);
                    break;
                case enemies_1.EnemyType.PATROL:
                    this.updatePatrol(enemy);
                    break;
                case enemies_1.EnemyType.HUNTER:
                    this.updateHunter(enemy, playerX, playerY);
                    break;
                case enemies_1.EnemyType.SNIPER:
    ...
    updateGuard(enemy, playerX, playerY) {
        ...
        if (distance < 150) { // Detection range
    ...
    updateHunter(enemy, playerX, playerY) {
        ...
        const dy = playerY - enemy.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < 300) { // Large detection range
            enemy.direction = Math.atan2(dy, dx);
            enemy.x += Math.cos(enemy.direction) * enemy.speed;
            enemy.y += Math.sin(enemy.direction) * enemy.speed;
        }
    ...
    updateSniper(enemy, playerX, playerY) {
        // Snipers don't move, but they track the player for targeting
        const dx = playerX - enemy.x;
        const dy = playerY - enemy.y;
        enemy.direction = Math.atan2(dy, dx);
    }
    ...
    renderObstacles() {
        ...
        this.obstacles.forEach(obstacle => {
            ...
            switch (obstacle.type) {
                case enemies_1.ObstacleType.WALL:
                    this.ctx.fillStyle = '#666666';
                    this.ctx.fillRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height);
                    break;
                case enemies_1.ObstacleType.SPIKES:
                    ...
                    break;
                case enemies_1.ObstacleType.LASER_BEAM:
                    this.ctx.fillStyle = '#FF0000';
                    this.ctx.fillRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height);
                    this.ctx.shadowColor = '#FF0000';
                    this.ctx.shadowBlur = 10;
                    break;
                case enemies_1.ObstacleType.ELECTRIC_FENCE:
    ...
    renderPowerUps() {
        ...
        this.powerUps.forEach(powerUp => {
            ...
            switch (powerUp.type) {
                case enemies_1.PowerUpType.SPEED_BOOST:
                    this.ctx.fillStyle = '#00FF00';
                    break;
                case enemies_1.PowerUpType.SHIELD:
                    this.ctx.fillStyle = '#0080FF';
                    break;
                case enemies_1.PowerUpType.INVISIBILITY:
                    this.ctx.fillStyle = '#8000FF';
                    break;
                case enemies_1.PowerUpType.HEALTH_POTION:
