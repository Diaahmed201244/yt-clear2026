...
road.frame = function () {
  ...
  if (Math.random() < 0.001) { // Rare spawn
    ...
    powerUp.style.background = 'yellow';
    powerUp.style.borderRadius = '50%';
    powerUp.style.left = Math.random() * (road.width - 20) + 'px';
    powerUp.style.bottom = '0px';
    powerUp.style.zIndex = '10';
    document.getElementById('game').appendChild(powerUp);
    
    // Animate power-up moving up
    var anim = powerUp.animate([
      { bottom: '0px', opacity: 1 },
      { bottom: road.height + 'px', opacity: 0 }
    ], {
      duration: 5000 / car.speed, // Adjust based on speed
      easing: 'linear'
    });
    ...
    setInterval(function() {
      if (powerUp.parentNode && !car.crashed) {
        var puRect = powerUp.getBoundingClientRect();
        var carRect = car.getBoundingClientRect();
        if (puRect.left < carRect.right && puRect.right > carRect.left &&
            puRect.top < carRect.bottom && puRect.bottom > carRect.top) {
          // Boost!
          car.maxSpeed += 1;
          setTimeout(function() { car.maxSpeed = 5; }, 5000); // 5 sec boost
          powerUp.remove();
          // Audio boost sound - increase frequency temporarily
          game.audio.oscillator.frequency.value *= 1.5;
          setTimeout(function() { game.audio.oscillator.frequency.value /= 1.5; }, 1000);
        }
      }
    }, 16);
  }
...
function updateHighLapDisplay() {
  var highLapElement = document.getElementById('highLap');
  if (!highLapElement) {
    highLapElement = document.createElement('div');
    highLapElement.id = 'highLap';
    highLapElement.innerHTML = '<a>H</a><a>' + game.highLap + '</a>';
    document.getElementById('panel').appendChild(highLapElement);
  } else {
    highLapElement.innerHTML = '<a>H</a><a>' + game.highLap + '</a>';
  }
}
lap.frame = function () {
  if (car.position <= 0) {
    ...
    car.easy += 0.5;
    car.position = 200;
    if (lap.value > game.highLap) {
      game.highLap = lap.value;
      localStorage.setItem('carRaceHighLap', game.highLap);
      updateHighLapDisplay();
    }
  }
  if (lap.value > 9) {
    alert("GAME OVER\n YOU WIN!!!\nHigh Lap: " + game.highLap);
    if (lap.value > game.highLap) {
      game.highLap = lap.value;
      localStorage.setItem('carRaceHighLap', game.highLap);
    }
...
game.init = function () {
  game.time = 0;
  game.highLap = localStorage.getItem('carRaceHighLap') ? parseInt(localStorage.getItem('carRaceHighLap')) : 0;
  car.init();
  cars.init();
  road.init();
  position.init();
