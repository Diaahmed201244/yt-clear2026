...
(function() {
    ...
    // Override the game end functions to notify dashboard
    const originalUpdateLifePoints = window.updateLifePoints;
    window.updateLifePoints = function(amount) {
        originalUpdateLifePoints(amount);
        if (amount <= 0) {
            // Player lost
            gameIntegration.gameOver({ score: 0, won: false });
        }
    };

    const originalIShoot = window.iShoot;
    window.iShoot = function(ennemy) {
        originalIShoot(ennemy);
        // Check if all enemies are dead
        if (!livingEnnemies().length) {
            // Player won
            gameIntegration.gameOver({ score: 100, won: true });
        }
    };
})();