...
// Simple SimplexNoise polyfill for perlin2
(function() {
  ...
  var noise = {
    ...
    simplex2: function(xin, yin) {
      ...
      var s = (xin + yin) * F2; // Skew the input space to determine which simplex cell we're in
      var i = Math.floor(xin + s); // integer part of x
      var j = Math.floor(yin + s); // integer part of y
      var t = (i + j) * G2;
      var X0 = i - t; // The x,y distances from the cell origin, unskewed.
      var Y0 = j - t;
      var x0 = xin - X0; // The x,y distances from the cell origin
      var y0 = yin - Y0;
      var i1, j1; // For the 2D case, the simplex shape is an equilateral triangle.
      var x1, y1; // Offsets for second (middle) corner of simplex in (x,y) unskewed coords
      var x2, y2; // Offsets for third (upper right) corner of simplex in (x,y) unskewed coords
      if (x0 > y0) {
      ...
      i = this.perm[i % 256];
      j = this.perm[j % 256];
      var gi0 = this.grad3[i + j % 3];
      var gi1 = this.grad3[i + i1 + (j + j1) % 3];
      var gi2 = this.grad3[i + (j + 1) % 3];
      var t0 = 0.5 - x0 * x0 - y0 * y0;
      if (t0 < 0) {
        n0 = 0.0;
      } else {
        t0 *= t0;
        n0 = t0 * t0 * (gi0[0] * x0 + gi0[1] * y0); // (x,y) of grad3 used for 2D gradient
      }
    ...
    seed: function(seed) {
      ...
      for (i = 0; i < 512; i++) perm[i + 256] = perm[i % 256];
    }
  };
  window.noise = noise; // Make global
})();
...
class Canvas {
  ...
  drawBall(ball) {
      ...
      if (!isFinite(rad) || !isFinite(offsetX) || !isFinite(offsetY)) {
        this.context.translate(-x, -y);
        return;
      }
      let grad = this.context.createRadialGradient(
      ...
      if (!ball.cue) {
        this.context.fillStyle = 'white';
        this.context.font = `bold ${rad * 1.5}px Arial`;
        this.context.textAlign = 'center';
        this.context.textBaseline = 'middle';
        this.context.fillText(ball.number.toString(), 0, 0);
      }
...
class Game {
  constructor({ world, canvas, sounds }) {
      this.highScore = localStorage.getItem('billiardHighScore') ? parseInt(localStorage.getItem('billiardHighScore')) : 0;
  ...
  updatePlayerDOM(player, current) {
    ...
    <span>${player.points}</span>
      ...
      if (player.number === 1 && current) {
        scoreHtml += `<span>High: ${this.highScore}</span>`;
      }
      return scoreHtml;
    }
  ...
  get aMachineBall() {
      ...
      // Prefer closest ball to cueball for better AI
      if (balls.length > 0) {
        balls.sort((a, b) => {
          let distA = Vector.magnitude(Vector.sub(this.cueball.body.position, a.body.position));
          let distB = Vector.magnitude(Vector.sub(this.cueball.body.position, b.body.position));
          return distA - distB;
        });
      }
      return balls[Math.floor(Math.random() * Math.min(3, balls.length))]; // Choose from top 3 closest
    }
