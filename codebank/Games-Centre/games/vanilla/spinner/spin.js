...
(function() {
    ...
    // For this spinner game, we'll call gameOver when the wheel stops spinning
    const originalTick = window.tick;
    window.tick = function() {
        originalTick();

        // Check if game has ended (wheel stopped spinning)
        if (window.mouse && !window.mouse.isSpinning && window.start3) {
            // Game has ended, notify dashboard
            gameIntegration.gameOver({
                score: 100, // Placeholder score
                won: true
            });
            window.start3 = false; // Reset game state
        }
    };
})();