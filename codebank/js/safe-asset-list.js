<<<<<<< HEAD
// safe-asset-list.js
// UV: SAFE-UI-UNIFY-2026-02-18
/* ===================================================
   Unified Safe Asset List Renderer - Full version
   - Preserves original 370 lines functionality
...
(function() {
  ...
  function renderSafeAssets(tab, container) {
    const snapshot = (window.__INDEXCB_ASSETS__) || (window.AssetBus && window.AssetBus.snapshot());
    if (!snapshot) {
  ...
  function renderList(tab, list, container) {
    ...
    if(!list || list.length===0){
      ...
      emptyMsg.textContent = `No ${TITLE_MAP[tab].toLowerCase()} yet.`; listContainer.appendChild(emptyMsg); return;
    }
    ...
    pageItems.forEach(item=>{
      ...
      row.textContent=item; listContainer.appendChild(row);
    });

    // Pagination controls
    const controls = document.createElement('div');
    controls.style.cssText='display:flex;justify-content:center;gap:10px;margin-top:12px;color:#94a3b8;';
    const prev = document.createElement('button'); prev.textContent='Prev'; prev.className='safe-tab-button';
    prev.disabled = window.SAFE_PAGE<=1; prev.onclick=()=>{ window.SAFE_PAGE=Math.max(1,window.SAFE_PAGE-1); renderSafeAssets(tab, container); };
    const next = document.createElement('button'); next.textContent='Next'; next.className='safe-tab-button';
    next.disabled = window.SAFE_PAGE>=maxPage; next.onclick=()=>{ window.SAFE_PAGE=Math.min(maxPage,window.SAFE_PAGE+1); renderSafeAssets(tab, container); };
    const info = document.createElement('span'); info.textContent=`Page ${window.SAFE_PAGE} / ${maxPage} (${list.length} items)`;
  ...
  function setupEventListeners(){
    function handleAssetUpdate(e){
      ...
      const container=document.querySelector(CODEBANK_CONTAINER_SELECTOR);
      if(container){ renderSafeAssets(tab,container); window.__SAFE_RENDER_PENDING__=false; }
      else window.__SAFE_RENDER_PENDING__=true;
    }
  ...
  // ========================
  // Init with guarded observer and retry
  // ========================
  function initSafeAssetList(){
    setupEventListeners();
    function render(){ const c=document.querySelector(CODEBANK_CONTAINER_SELECTOR); if(c){ renderSafeAssets(window.ACTIVE_ASSET_TAB||'codes', c); } }
    function initObserver(root){ const observer=new MutationObserver(()=>{ render(); }); observer.observe(root,{childList:true,subtree:true}); try{ console.log('[SafeAssetList] Observer attached'); }catch(_){} }
    function waitForSafeList(attempt=0){ const root=document.querySelector('#safe-list'); if(root && root instanceof Node){ initObserver(root); return; } if(attempt<20){ setTimeout(()=>waitForSafeList(attempt+1),100); } else { try{ console.warn('[SafeAssetList] Root never found.'); }catch(_){} } }
    function waitForBody(attempt=0){ const b=(document&&document.body)||null; if(b && b instanceof Node){ const observer=new MutationObserver(()=>{ const c=document.querySelector(CODEBANK_CONTAINER_SELECTOR); if(c){ renderSafeAssets(window.ACTIVE_ASSET_TAB||'codes',c); observer.disconnect(); waitForSafeList(); if(window.__SAFE_RENDER_PENDING__) window.__SAFE_RENDER_PENDING__=false; } }); observer.observe(b,{childList:true,subtree:true}); return; } if(attempt<20){ setTimeout(()=>waitForBody(attempt+1),100); } else { try{ console.warn('[SafeAssetList] Body never ready.'); }catch(_){} } }
    const container = document.querySelector(CODEBANK_CONTAINER_SELECTOR);
    if(container){ renderSafeAssets(window.ACTIVE_ASSET_TAB||'codes', container); waitForSafeList(); }
    else { waitForBody(); }
  }

  // Wait for DOMContentLoaded before initializing
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSafeAssetList);
  } else {
    initSafeAssetList();
  }
  ...
  // ========================
  if(window.Bankode && window.AssetBus && !Bankode._uiBridgeAttached){
    ...
    // إعادة بث snapshot الموجود عند Init
  ...
  // Public API
  // ========================
  window.SafeAssetList={ render: renderSafeAssets, switchTab: switchAssetTab, getCurrentTab:()=>window.ACTIVE_ASSET_TAB };

  console.log('[SafeAssetList] Unified Safe Asset List renderer loaded');
})();
=======
...
(function() {
  ...
  // Main Renderer with proper loading state handling
   function renderSafeAssets(tab, container, providedSnapshot) {
    ...
    if (!cont) {
      ...
      // FIXED: Always hide loading even if container is missing
