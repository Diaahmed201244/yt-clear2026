...
async function run(){
  ...
  const genRes = await page.evaluate(async () => {
    ...
    const res = await fetch('/api/sqlite/codes', {
      method: 'POST',
      credentials: 'include',
  ...
  const neonCodes = await page.evaluate(async () => { const r = await fetch('/api/sqlite/codes', { credentials:'include' }); return r.ok ? await r.json() : null })
  if(!neonCodes || neonCodes.status!=='success') throw new Error('GET /api/sqlite/codes failed after write')
  ...
  }, BASE_URL + '/api/sqlite/codes')
  console.log('GET /api/sqlite/codes status:', neonRes.status)
  trace = await getUVTrace(page)
  ...
  const ensureAHasCode = await page.evaluate(async () => {
    const r = await fetch('/api/sqlite/codes', { method:'GET', credentials:'include' }); const j = await r.json(); const c = (typeof j.count==='number'? j.count : (Array.isArray(j.rows)? j.rows.length : 0));
    if (c>0) return true;
    const code = 'E2ETEST' + Date.now() + 'P1'
    const p = await fetch('/api/sqlite/codes', { method:'POST', credentials:'include', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ code, suffix:'P1', ts: Date.now() }) })
    return p.ok
  })
  ...
  const neonAB = await page.evaluate(async (url, a, b) => { const r = await fetch(url + '?user1=' + a + '&user2=' + b); const j = await r.json(); return j && j.rows ? j.rows : [] }, BASE_URL + '/api/sqlite/balances', whoA.id, whoB.id)
  ...
  await page.waitForTimeout(400)
  const neonAB2 = await page.evaluate(async (url, a, b) => { const r = await fetch(url + '?user1=' + a + '&user2=' + b); const j = await r.json(); return j && j.rows ? j.rows : [] }, BASE_URL + '/api/sqlite/balances', whoA.id, whoB.id)
