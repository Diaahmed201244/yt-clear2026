...
async function claimReward(reward) {
    ...
    try {
        ...
        if (!code || !window.AssetBus || typeof window.AssetBus.addAsset !== 'function') {
            throw new Error('AssetBus or code generation failed');
        }
        ...
        // STEP 6: Cleanup (always runs after DONE)
        // Use setTimeout to ensure success message is visible before cleanup
        setTimeout(() => {
            cleanupExtraModeUI();
            resetExtraProgress();
            hideRewardNotification();
...
function formatTime(milliseconds) {
    ...
    // Format as xx:xx:xx:xxx:x (seconds:minutes:hours:days:years)
    ...
    return `${formattedSeconds}:${formattedMinutes}:${formattedHours}:${formattedDays}:${formattedYears}`;
}
...
if (!document.getElementById('extra-mode-styles')) {
    ...
    styleElement.innerHTML = extraModeStyles;
