...
// UV: STORAGE-ADAPTER-2026-03-26 - FALLBACK CACHE ONLY
// ⚠️  ROLE CHANGE: This is now a LOCAL FALLBACK CACHE only.
//      All authoritative asset operations go through ACC (window.ACC).
//      This file handles: local IDB persistence, offline code storage, pending-tx queue.
//      It does NOT write asset balances. ACC is the single source of truth.
...
(function() {
  ...
  const WebStorage = {
    ...
    async getCodes(skipIntegrity = false) {
      ...
      // 🛡️ BACKGROUND SYNC: Fetch from server if online, but return local cache first (from actly.md)
      if (isAuth && navigator.onLine) {
        this.syncWithServerInBackground().catch(() => {});
      }
        ...
        try {   
          ...
          if (response.ok) {
            ...
            if (data.success && Array.isArray(data.codes)) {
              console.log('[VAULT] Background sync successful:', data.codes.length);
              await this._updateLocalCache(data.codes);
              // Notify system that assets updated
              window.dispatchEvent(new CustomEvent('assets:updated'));
            }
          }
        } catch (e) {
          console.warn('[VAULT] Background sync failed:', e.message);
        }
    },
  ...
  // --- PUBLIC INTERFACE ---
   const StorageAdapter = {
     async saveCode(input, type = 'normal', meta = {}) {
       ...
       // 2. Defer asset tracking to ACC (single source of truth)
       //    Direct SQLite balance writes are bypassed; ACC manages balances.
       if (window.ACC && typeof window.ACC.earn === 'function' && window.FEATURES?.USE_ACC) {
         window.ACC.earn(1, 'codes', { source: 'code_save', code })
           .catch(err => console.warn('[StorageAdapter] ACC earn notify failed:', err));
       }
  ...
  console.log('✅ [StorageAdapter] Initialized - Fallback Cache Mode (ACC is source of truth)');
})();
