...
<<<<<<< HEAD
=======
  // 🧪 DIAGNOSTIC: Track all reload calls
  (function() {
      ...
      // 🛡️ DO NOT OVERWRITE location.reload OR location.replace
      // They are read-only in most modern browsers and cause TypeErrors
      console.log('[BankodeCore] Location diagnostic ready (passive only)');
  })();
...
=======
    ...
    async fetchCodesFromServer() {
        try {
            const token = this.sessionId || localStorage.getItem('session_token');
            ...
            console.log('[VAULT] Fetched codes:', Array.isArray(codes) ? codes.length : 'unknown');
...
=======
    ...
    async _initFromStorage(){
      ...
      // 🛡️ SSoT: Load full codes list from IDB or server on init
      try {
        const online = typeof navigator !== 'undefined' && navigator.onLine === true;
        const isAuth = !!(window.Auth && typeof window.Auth.isAuthenticated === 'function' && window.Auth.isAuthenticated());
        ...
        let all = [];
        if (online && isAuth) {
           console.log('[Bankode] Attempting to fetch authoritative codes from server...');
           all = await this.fetchCodesFromServer();
        }
        ...
        // 🛡️ FIX: Merge pending sync queue into codes list for immediate display (from actly.md)
        const pendingCodes = (this._syncQueue || []).map(item => item.code);
        if (pendingCodes.length > 0) {
          console.log('[Bankode] Merging', pendingCodes.length, 'pending codes into display list');
          // Add pending codes if they aren't already in the list
          pendingCodes.forEach(pc => {
            if (!this.codes.includes(pc)) {
              this.codes.push(pc);
            }
          });
        }
