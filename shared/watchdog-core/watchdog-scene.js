...
export class WatchDogScene {
  ...
  async loadGLBModel() {
    ...
    const modelPath = '/shared/watchdog-core/3d-watchdog.glb?v=' + Date.now();
    
    console.log('[WatchDogScene] 🛡️ Loading Authoritative GLB model from:', modelPath);
    
    // WATCHDOG LOAD TIMEOUT (FIX 3)
    ...
    try {   
      const gltf = await new Promise((resolve, reject) => {
        ...
        timeoutId = setTimeout(() => {
          console.warn('[WatchDogScene] Load timeout - forcing ready state');
          reject(new Error('Model load timeout'));
        }, WATCHDOG_LOAD_TIMEOUT);
