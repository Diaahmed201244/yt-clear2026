...
(function() {
  ...
  const ServiceManager = {
    ...
    deactivateService(serviceId) {
      ...
      // 🚨 GPU FIX: Destroy iframe instead of hiding (saves ~400-800MB GPU)
      service.iframe.style.opacity = '0';
      
      setTimeout(() => {
        // Clear src to release video/GPU resources
        service.iframe.src = 'about:blank';
        
        // Remove iframe from DOM after brief cleanup
        setTimeout(() => {
          service.iframe.remove();
      ...
      console.log('[ServiceManager] GPU: Destroyed iframe for', serviceId);
    },
    ...
    async destroyService(serviceId) {
      ...
      // 🚨 GPU FIX: Clear iframe src before removing (releases GPU memory)
      if (service.iframe) {
        service.iframe.src = 'about:blank';
      }
      ...
      console.log('[ServiceManager] GPU: Destroyed service completely:', serviceId);
    },
